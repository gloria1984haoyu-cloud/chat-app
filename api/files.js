...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const path = id
        ? `files?id=eq.${encodeURIComponent(id)}&select=*`
        : 'files?select=*&order=created_at.asc';
      const upstream = await fetch(`${SB_URL}/rest/v1/${path}`, { headers: sbHeaders() });
      if (!upstream.ok) return proxyError(upstream, res);
      const data = await upstream.json();
      return sendJson(res, 200, Array.isArray(data) ? data : []);
    } catch (e) {
      return sendJson(res, 502, { error: { message: e.message || '文件云同步失败' } });
    }
  }

  if (req.method === 'POST') {
    const incoming = Array.isArray(req.body) ? req.body : [req.body];
    const payload = incoming.map(safeFile).filter(file => file.id);
    if (payload.length === 0) return sendJson(res, 400, { error: { message: '缺少文件 ID' } });

    try {
      const upstream = await fetch(`${SB_URL}/rest/v1/files`, {
        method: 'POST',
  ...
  if (req.method === 'DELETE') {
    const id = req.query?.id;
    if (!id) return sendJson(res, 400, { error: { message: '缺少文件 ID' } });

    try {
      const upstream = await fetch(`${SB_URL}/rest/v1/files?id=eq.${encodeURIComponent(id)}`, {
        method: 'DELETE',
